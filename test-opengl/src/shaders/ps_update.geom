...
#define PARTICAL_TYPE_LAUCHER 0
#define PARTICAL_TYPE_SHELL 1
#define PARTICAL_TYPE_SECONDARY_SHELL 2
...
void main(){
	...
	if (gs_out.type == PARTICAL_TYPE_LAUCHER){
	...
	else if(gs_out.type == PARTICAL_TYPE_SHELL){
		if(gs_out.age >= g_shell_life_time){
			...
			gs_out.position += gs_out.velocity * g_delta_time / 1000;
			gs_out.velocity += vec3(0, -9.8, 0) * g_delta_time / 1000;
			EmitVertex();
			EndPrimitive();
		}
	}
